...
export function PostCard({ postRef }: PostCardProps) {
    const post = ({
        pull: function(param) {
            return param.PostCardFragment;
        }
    }).pull(postRef);
    return <article className="flex flex-col max-w-2xl mx-auto">

      <Link href={`/post/${post.id}`} className="text-blue-600 underline visited:text-gray-700">
...
export function FallbackCard() {
    return <article className="flex flex-col max-w-2xl mx-auto space-y-1">
